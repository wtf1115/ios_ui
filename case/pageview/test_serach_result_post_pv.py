...
class Search_result_post_pv(unittest.TestCase):
    log = Log()

    def test_search_result_post_pv(self):
        """
        搜索结果页-帖子
        """
        ...
        with GMdriver() as driver:
            ...
            time.sleep(5)
            driver.click_tractate_statictext()
            time.sleep(1)
            driver.background_app(3)

        result = mysql_test.query(action='page_view', page_name='search_result_post', event_time=begin_date)
